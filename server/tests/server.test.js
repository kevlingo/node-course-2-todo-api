...
describe('DELETE /todos/:id', () => {
  ...
  it("should not remove a todo the user didn't create", done => {
...
describe('PATCH /todos/:id', () => {
  ...
  it("should not update the todo if it wasn't create by the user", done => {
    let id = todos[0]._id.toHexString();
...
describe('POST /users/login', () => {
  it('should login user and return auth token', done => {
    ...
    request(app)
      ...
      .send({ email, password })
      .expect(200)
      .expect(res => expect(res.header['x-auth']).toBeTruthy())
      .end((err, res) => {
        if (err) {
          return done(err);
        }
  ...
  it('should reject invalid login', done => {
    ...
    request(app)
      ...
      .send({ email, password })
      .expect(401)
      .expect(res => expect(res.header['x-auth']).toBeFalsy())
      .end((err, res) => {
        if (err) {
          return done(err);
        }
